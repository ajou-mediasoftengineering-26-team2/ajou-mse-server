...
import team2.mse.ajou.server.domain.shared.LobbyState;
...
public class FrdbLobbyData {
    ...
    public static FrdbLobbyData from(LobbyData lobbyData) {
        Map<String, FrdbPlayerData> players = lobbyData
                .getPlayers()
                .stream()
                .collect(Collectors.toMap(playerData -> playerData.getId().toString(), FrdbPlayerData::from));

        return new FrdbLobbyData(
                players,
                lobbyData.getState(),
                lobbyData.getCurrentTurn(),
                lobbyData.getCurrentPlayer(),
                // lobbyData.getCountdownStartTime().toLocalDateTime(),
                lobbyData.getCountdownStartTime().format(TIME_FORMATTER),
                lobbyData.getCountdownSec()
        );
    }
}
